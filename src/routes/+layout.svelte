<script lang="ts">
	...
	$effect(() => {
		const trackUrl = $currentSong.audioUrl; 
		if (audioElement) {
			if ($isPlaying) {
				setTimeout(() => audioElement?.play().catch(err => console.log("Autoplay blocked:", err)), 50);
			} else {
				audioElement?.pause();
			}
		}
	...
	// Removed the e? and replaced with standard JS parameter handling
...
<div class="h-[100dvh] w-full flex flex-col relative bg-zinc-950 text-zinc-50 antialiased selection:bg-zinc-800">
	...
	>
		...
		<div class="flex items-center justify-end gap-4 w-5/12 pt-1">
			<button onclick={(e) => playNext(e)} class="p-2 text-zinc-400 hover:text-white transition active:scale-90 hidden sm:block">
			...
			<button onclick={togglePlay} class="w-12 h-12 flex items-center justify-center rounded-full bg-zinc-50 text-zinc-950 hover:bg-white active:scale-95 transition-all">
				{#if $isPlaying}
...
<div 
	class="fixed inset-0 z-[100] bg-zinc-950 flex flex-col transition-transform duration-500 ease-in-out {isFullScreen ? 'translate-y-0' : 'translate-y-full'}"
>
	<div class="flex items-center justify-between px-6 pt-12 pb-4">
		<button onclick={() => isFullScreen = false} class="p-2 -ml-2 text-zinc-400 hover:text-white transition rounded-full hover:bg-zinc-800/50">
	...
	<div class="flex-1 flex flex-col items-center justify-center px-8">
		<img src={$currentSong.coverArt} alt="Album Art" class="w-full max-w-[350px] aspect-square rounded-3xl shadow-2xl object-cover mb-12" />
		
		<div class="w-full max-w-[350px] flex flex-col items-start mb-8">
		...
		<div class="w-full max-w-[350px] flex items-center justify-between">
			<button onclick={(e) => skipTime(-5, e)} class="text-zinc-400 hover:text-white transition active:scale-90" aria-label="-5 Seconds">
			...
			<button onclick={(e) => playPrevious(e)} class="text-zinc-100 hover:text-zinc-300 transition active:scale-90">
			...
			<button onclick={togglePlay} class="w-20 h-20 flex items-center justify-center rounded-full bg-white text-black active:scale-95 transition-transform shadow-[0_0_40px_rgba(255,255,255,0.2)]">
				{#if $isPlaying}
			...
			<button onclick={(e) => playNext(e)} class="text-zinc-100 hover:text-zinc-300 transition active:scale-90">
